...
MAPPING = make_mapping("bloomberg", "BRC_SHOP_PRICE_FOOD_YOY", "TESTBRCFOOD Index", "PX_LAST")
QUERY = {"service": "//blp/refdata", "security": "TESTBRCFOOD Index"}
...
def test_bloomberg_parses_a_historical_response() -> None:
    field_data = FakeElement(array=[_point(date(2024, 1, 31), 2.5), _point(date(2024, 2, 29), 2.7)])
    ...
    assert response.series_id == "BRC_SHOP_PRICE_FOOD_YOY"
...
def test_bloomberg_query_records_the_monthly_request_shape() -> None:
    ...
    assert query["security"] == "TESTBRCFOOD Index"
...
LSEG_MAPPING = make_mapping("lseg", "BRC_SHOP_PRICE_FOOD_YOY", "TESTBRCFOOD=ECI", "VALUE")


def test_lseg_parses_records_including_a_reported_release_instant() -> None:
    records = [
        {"date": "2024-01-31", "VALUE": 2.5, "release_timestamp": "2024-02-06T00:01:00"},
        {"date": "2024-02-29", "VALUE": 2.7},
    ]
    response = extract_lseg.parse_history_records(
        records, LSEG_MAPPING, {"universe": "TESTBRCFOOD=ECI"}, RETRIEVED
    )
...
def test_lseg_null_values_survive_as_none_rather_than_zero() -> None:
    """A missing month is missing. Coercing it to 0.0 would invent a rate of zero."""
    response = extract_lseg.parse_history_records(
...
def test_lseg_errors_map_to_distinct_exceptions(message: str, expected: type[Exception]) -> None:
    classified = extract_lseg.classify_error(RuntimeError(message), "TESTBRCFOOD=ECI", "VALUE")
...
def test_lseg_reduces_a_single_column_frame_even_when_the_name_differs() -> None:
    frame = FakeFrame(["Shop Price"], [date(2024, 1, 31)], {"Shop Price": [2.5]})
    records = extract_lseg.dataframe_to_records(frame, "VALUE")
    assert records == [{"date": date(2024, 1, 31), "VALUE": 2.5}]
...
def test_lseg_refuses_to_guess_between_several_columns() -> None:
    ...
    frame = FakeFrame(
        ["Shop Price", "Food"], [date(2024, 1, 31)], {"Shop Price": [2.5], "Food": [3.1]}
    )
    with pytest.raises(FieldNotFoundError):
