...
Every fixture value here is invented. BRC data is licensed and must never enter
this repository, so the fixtures reproduce the *shape* of a vendor response —
monthly stamps, a rate-like magnitude, the missing values and the release
timestamps a provider does or does not supply — and nothing of its content. A
test that needed a real BRC number would be a test that could not be committed.
"""
...
def synthetic_rows(count: int = 6, start: float = 1.0, step: float = 0.25) -> list[VendorRow]:
    """Build `count` monthly rows with invented, rate-shaped values."""
    rows = []
...
def bloomberg_response(
    series_id: str = "BRC_SHOP_PRICE_FOOD_YOY",
    rows: list[VendorRow] | None = None,
    identifier: str = "TESTBRCFOOD Index",
    field: str = "PX_LAST",
...
def lseg_response(
    series_id: str = "BRC_SHOP_PRICE_FOOD_YOY",
    rows: list[VendorRow] | None = None,
    identifier: str = "TESTBRCFOOD=ECI",
    field: str = "VALUE",
) -> VendorSeriesResponse:
