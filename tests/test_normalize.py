...
def test_a_mid_month_stamp_is_rejected_rather_than_truncated() -> None:
    ...
    with pytest.raises(ValueError, match="monthly series"):
...
def test_a_non_canonical_series_id_is_refused() -> None:
    ...
    response = bloomberg_response(series_id="BLOOMBERG_BRC_FOOD")
...
def test_both_providers_produce_the_same_canonical_series() -> None:
    ...
    assert {o.series_id for o in from_bloomberg} == {"BRC_SHOP_PRICE_FOOD_YOY"}
...
def test_cross_check_reports_coverage_gaps() -> None:
    """A provider missing months is a finding even when the shared months agree."""
