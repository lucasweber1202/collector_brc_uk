...
from tests.conftest import bloomberg_response, lseg_response, make_mapping, synthetic_rows

SERIES = "BRC_SHOP_PRICE_FOOD_YOY"
REGISTRY = {
    (SERIES, "bloomberg"): make_mapping("bloomberg", SERIES, "TESTBRCFOOD Index", "PX_LAST"),
    (SERIES, "lseg"): make_mapping("lseg", SERIES, "TESTBRCFOOD=ECI", "VALUE"),
}
...
def test_a_revision_is_first_seen_and_never_reuses_the_original_release(engine: Engine) -> None:
    ...
    rows = [
        VendorRow(date(2024, 1, 1), 2.5, release_timestamp=datetime(2024, 2, 6, 0, 1)),
        VendorRow(date(2024, 2, 1), 2.7, release_timestamp=datetime(2024, 3, 5, 0, 1)),
    ]
...
def test_a_vendor_backfill_is_first_seen_not_backdated(engine: Engine) -> None:
    """Bloomberg serving twenty years today does not mean we knew it back then."""
    collected_at = datetime(2026, 9, 17, 9, tzinfo=UTC)
...
def test_a_provider_reported_instant_beats_first_seen(engine: Engine) -> None:
    rows = [VendorRow(date(2024, 1, 1), 2.5, release_timestamp=datetime(2024, 2, 6, 0, 1))]
    _run(engine, _collect(rows, provider="lseg"), datetime(2026, 9, 17, 9, tzinfo=UTC))
...
def test_an_observation_reconstructs_its_full_provenance(engine: Engine) -> None:
    ...
    assert row["original_publisher"] == "British Retail Consortium"
    assert row["delivery_provider"] == "bloomberg"
    assert row["vendor_series_id"] == "TESTBRCFOOD Index"
...
def test_a_cross_check_disagreement_is_surfaced_for_the_run_to_refuse() -> None:
    ...
    data = _collect(synthetic_rows(count=3))
    data.cross_check_differences = ["BRC_SHOP_PRICE_FOOD_YOY 2024-01-01: bloomberg=2.5 lseg=2.9"]
