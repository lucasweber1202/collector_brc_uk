...
``official_timestamp``
    ...
    read off BRC's website does not, however reliable it looks.
``official_date``
    A publication date was reported, but only to day precision. It is stamped at
    00:01 Europe/London, the Shop Price Monitor's published release time,
    converted to UTC.
``archived_release``
...
and LSEG will happily serve twenty years of BRC history in one call today. That
...
def attribute_release(
    ...
    2. The provider reported a publication date only. That is `official_date`,
       stamped at the Shop Price Monitor's 00:01 Europe/London release time and
       converted to UTC through the tz database, so GMT and BST both come out
       right.
    ...
    does not know when a 2011 observation reached the vendor, and claiming to
