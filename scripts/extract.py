"""Collect the BRC Shop Price Monitor through a licensed delivery provider.
...
def collect() -> CollectedData:  # pragma: no cover - requires a live vendor session
    ...
    if not mappings:
        raise PendingVendorDiscoveryError(primary, "every canonical BRC series")
    ...
    try:
        ...
        if not fallback_mappings:
            raise PendingVendorDiscoveryError(
                FALLBACK_PROVIDER, "every canonical BRC series"
            ) from error
...
def run_cross_check(  # pragma: no cover - requires a live vendor session
    ...
    a failure, because two licensed routes to the same BRC statistic returning
