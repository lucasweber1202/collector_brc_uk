"""Read BRC history from Bloomberg through the official blpapi library.

Bloomberg is a licensed *delivery provider* here, never the publisher. The
statistic stays a British Retail Consortium publication; Bloomberg is the route
the desk is entitled to use.
...
def parse_historical_response(
    ...
    response *shapes* — never against real BRC values, which are licensed and
