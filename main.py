"""Run the standalone collector_brc_uk pipeline.

BRC is the economic publisher. Bloomberg and LSEG are licensed delivery
...
def _parse_args(argv: list[str] | None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Collect the BRC Shop Price Monitor.")
...
def availability_rows(
    ...
    A later revision of an already stored month must never reuse the original
...
def collect_source(engine: Engine, start_date: date) -> None:
    ...
    if data.cross_check_differences:
        # Two licensed routes to the same BRC statistic disagreeing means one of
